...
def conDb(database:str,host='localhost',port=3306,user='root',password='rootroot',charset='utf8mb4'):
    db = pymysql.connect(
    host=host,
    port=port,
    user=user,  # 在这里输入用户名
    ...
    return db
def db_login():
    db = conDb(database='agency', password='rootroot')
    cursor = db.cursor()
    return db,cursor
...
class HttpRequestPacket(object):
    ...
    def __parse(self, data):
        ...
        i0 = data.find(b'\r\n') # 请求行与请求头的分隔位置
        i1 = data.find(b'\r\n\r\n') # 请求头与请求数据的分隔位置
        x0 = data.find(b'Authorization-Token')
        if(x0!=-1):
            x1 = data[x0:].find(b'\r\n')
            self.token=data[x0:x0+x1]
        else:
            self.token=b''
        # 请求行 Request-Line
...
class SimpleHttpProxy(object):
    ...
    def __proxy(self, socket_client):
        ...
        server_host, server_port = '192.168.209.130', 80
        # print(http_packet.req_uri)
        ...
        # 使用select异步处理，不阻塞
        self.__nonblocking(socket_client, socket_server,http_packet.token)

    def __nonblocking(self, socket_client, socket_server,token):
        '''
        使用select实现异步处理数据
            ...
            try:
                ...
                for tmp_socket in rlist:
                    ...
                    # socket_server状态为readable, 当前接收的数据来自服务端
                        ...
                        i0=data.find(b'\r\n')
                        if(data[:i0]==b'HTTP/1.1 200 OK'):      #
                            pass
                        else:
                            try:
                                data=make_watermark(data,token)
                            except:
                                print("not data")
                        print(8)
